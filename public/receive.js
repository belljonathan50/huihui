...
//           vid.src= "lib/vids/"+thispath+"/"+deuxieme+'.mp4';
//           console.log(vid.src);
//           break;
...
function connect(url, videopath, videoID) {
  let host = url ? url : location.origin.replace(/^http/, 'ws');
  var ws = new WebSocket(host);
  ws.onmessage = function(msg) {
    var response = msg.data;
    console.log("response "+response);
    ...
    switch (first) {
      ...
      case "tune":
          console.log(deuxieme);
          vid.src= videopath+"/"+deuxieme+'.mp4';
          console.log(vid.src);
          break;
